...
test.describe('Buyer Checkout Flow E2E', () => {
  ...
  test('step 1: Landing page loads correctly', async ({ page }) => {
    ...
    // Check for main CTA - varies by language (Thai/English)
    const getStartedBtn = page.getByRole('link', { name: /get started free|เริ่มใช้ฟรี/i })
    await expect(getStartedBtn).toBeVisible()
    
    // Check for features section
    const featuresHeading = page.locator('h2').filter({ hasText: /features|ทุกอย่างที่คุณต้องการ/i })
  ...
  test('step 2: Login page loads with auth options', async ({ page }) => {
    ...
    const loginHeading = page.locator('h1').filter({ hasText: /welcome back|ยินดีต้อนรับ/i })
    await expect(loginHeading).toBeVisible()
    
    // Should show OAuth buttons (Google and GitHub)
  ...
  test('step 5: Seller orders page shows order data', async ({ page }) => {
    await page.goto('http://localhost:3000/seller/orders')
  ...
  test('step 6: Stripe checkout endpoint exists', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:3000/api/stripe/checkout', {
      data: {
        userId: 'test-user-id',
  ...
  test('step 7: Game accounts seller page works', async ({ page }) => {
    await page.goto('http://localhost:3000/seller/game-accounts')
  ...
  test('step 9: Builder new page behavior', async ({ page }) => {
    await page.goto('http://localhost:3000/builder/new')
  ...
  test('summary: Verify all key pages accessible', async ({ page }) => {
    ...
    for (const p of pages) {
      const response = await page.goto(`http://localhost:3000${p.url}`)
