...
- Name: buyer-checkout.spec.ts >> Buyer Checkout Flow E2E >> step 2: Login page loads with auth options
- Location: e2e/buyer-checkout.spec.ts:41:7
...
Locator: locator('h1').filter({ hasText: /welcome back|ยินดีต้อนรับ/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h1').filter({ hasText: /welcome back|ยินดีต้อนรับ/i })
