import type { Translations } from './en'

export const th: Translations = {
  // Nav
  nav: {
    features: 'ฟีเจอร์',
    themes: 'ธีม',
    pricing: 'ราคา',
    login: 'เข้าสู่ระบบ',
    getStarted: 'เริ่มใช้ฟรี',
  },

  // Hero
  hero: {
    badge: 'ฟรี 100% ตั้งแต่เริ่มต้น — ไม่ต้องใช้บัตรเครดิต',
    title: 'พอร์ตโฟลิโอสำหรับ Developer',
    titleHighlight: 'ภายในไม่กี่นาที ไม่ใช่หลายวัน',
    subtitle: 'สร้างพอร์ตโฟลิโอที่สวยงามเพื่อโชว์ผลงานของคุณ เลือกจากธีม 15+ แบบ ปรับแต่งด้วยโค้ดได้ และ deploy ได้ทันที',
    createFree: 'สร้างพอร์ตโฟลิโอฟรี',
    browseThemes: 'ดูธีม',
    socialProof: 'นักพัฒนากว่า 2,000+ คนใช้แล้ว',
    fromText: 'จากกรุงเทพถึงเบอร์ลิน — นักพัฒนาทั่วโลกใช้ PortGen',
  },

  // Features
  features: {
    title: 'ทุกอย่างที่คุณต้องการ',
    subtitle: 'สร้างมาสำหรับ developer โดย developer ไม่มี drag-and-drop ให้ปวดหัว',
    items: {
      themes: {
        title: 'ธีมสวยๆ 15+ แบบ',
        desc: 'ตั้งแต่ minimal ถึง bold มี dark mode เป็นค่าเริ่มต้น ทุกธีมออกแบบอย่างดี',
      },
      semiCode: {
        title: 'ปรับแต่งด้วย Semi-Code',
        desc: 'ไม่พอใจ? แก้ JSX ได้เลย ควบคุมโค้ดพอร์ตโฟลิโอได้ทั้งหมด',
      },
      fast: {
        title: 'โหลดเร็วสุดๆ',
        desc: 'Static generation ทำให้พอร์ตโฟลิโอโหลดในไม่ถึง 100ms SEO ดีเยี่ยม',
      },
      hosting: {
        title: 'โฮสต์ฟรี',
        desc: 'พอร์ตโฟลิโอของคุณอยู่ที่ yourname.portgen.com ต่อ custom domain ได้ในแพลน Pro',
      },
      github: {
        title: 'เชื่อมต่อ GitHub',
        desc: 'เข้าสู่ระบบด้วย GitHub ดึง repo มาแสดงอัตโนมัติ',
      },
      export: {
        title: 'Export ไปที่ไหนก็ได้',
        desc: 'Export เป็น HTML/CSS โฮสต์บน Vercel, Netlify หรือที่ไหนก็ได้',
      },
    },
  },

  // Themes
  themes: {
    title: 'เลือกสไตล์ของคุณ',
    subtitle: 'มี 3 ธีมพร้อมใช้แล้ว อีก 12+ ธีมกำลังจะมา',
    free: 'ฟรี',
    pro: 'Pro',
    gradientDark: {
      name: 'Gradient Dark',
      desc: 'Gradient เด่นชัด สไตล์ทันสมัย',
    },
    minimalLight: {
      name: 'Minimal Light',
      desc: 'สะอาด เป็นมืออาชีพ อ่านง่าย',
    },
    brutalist: {
      name: 'Brutalist',
      desc: 'เด่น เรียบง่าย โดดเด่น',
    },
  },

  // Pricing
  pricing: {
    title: 'ราคาง่ายๆ',
    subtitle: 'เริ่มฟรี อัพเกรดเมื่อต้องการ',
    free: 'ฟรี',
    pro: 'Pro',
    perMonth: '/เดือน',
    billedMonthly: 'เก็บเงินรายเดือน',
    foreverFree: 'ฟรีตลอดไป',
    popular: 'ได้รับความนิยมสูงสุด',
    getStarted: 'เริ่มฟรี',
    startProTrial: 'ทดลองใช้ Pro ฟรี',
    cancelAnytime: 'ทดลองฟรี 7 วัน • ยกเลิกได้ทุกเมื่อ',
    features: {
      portfolio: '1 พอร์ตโฟลิโอ',
      themes3: '3 ธีม',
      subdomain: 'subdomain บน portgen.com',
      basicCustom: 'ปรับแต่งพื้นฐาน',
      exportHtml: 'Export HTML',
      community: 'ช่วยเหลือจากชุมชน',
      unlimited: 'พอร์ตโฟลิโอไม่จำกัด',
      themes15: '15+ ธีม',
      customDomain: 'Custom Domain',
      removeBranding: 'ลบแบรนด์ PortGen',
      analytics: 'แดชบอร์ดวิเคราะห์',
      priority: 'สนับสนุนลำดับความสำคัญสูง',
      earlyAccess: 'เข้าถึงธีมใหม่ก่อน',
    },
  },

  // CTA
  cta: {
    title: 'พร้อมที่จะโดดเด่นหรือยัง?',
    subtitle: 'ร่วมกับนักพัฒนากว่า 2,000+ คนที่ใช้ PortGen ในการโชว์ผลงาน',
    button: 'สร้างพอร์ตโฟลิโอตอนนี้',
  },

  // Footer
  footer: {
    copyright: '© 2025 PortGen. สร้างสำหรับนักพัฒนาทั่วโลก',
  },

  // Login
  login: {
    welcomeBack: 'ยินดีต้อนรับกลับ',
    continueToDashboard: 'เข้าสู่ระบบเพื่อไปยังแดชบอร์ดของคุณ',
    tryDemo: 'ลองโหมด Demo',
    tryDemoDesc: 'สำรวจโดยไม่ต้องมีบัญชี',
    demoFeatures: {
      portfolios: '3 พอร์ตโฟลิโอ',
      portfoliosPre: 'สร้างไว้แล้ว',
      customThemes: 'ธีมพิเศษ',
      customThemesUnlock: 'ปลดล็อกแล้ว',
      allFeatures: 'ทุกฟีเจอร์',
      allFeaturesAvailable: 'พร้อมใช้งาน',
    },
    enterDemo: 'เข้า Demo Dashboard',
    noAccountNeeded: 'ไม่ต้องมีบัญชี ใช้ข้อมูลตัวอย่างเท่านั้น',
    orContinueWith: 'หรือเข้าสู่ระบบด้วยอีเมล',
    continueWithGoogle: 'ดำเนินการต่อด้วย Google',
    continueWithGithub: 'ดำเนินการต่อด้วย GitHub',
    emailRequired: 'ต้องระบุอีเมล',
    proPlan: 'แพลน Pro',
    checkEmail: 'ตรวจสอบอีเมลของคุณ',
    magicLinkSent: 'เราส่ง magic link ไปที่',
    clickToSignIn: 'คลิกลิงก์เพื่อเข้าสู่ระบบ',
    sendMagicLink: 'ส่ง Magic Link',
    termsNote: 'เมื่อเข้าสู่ระบบ คุณตกลงกับข้อกำหนดการใช้งานและนโยบายความเป็นส่วนตัว',
    developersUsing: 'นักพัฒนาใช้งานอยู่',
  },

  // Dashboard
  dashboard: {
    title: 'พอร์ตโฟลิโอของคุณ',
    createFirst: 'สร้างพอร์ตโฟลิโอแรกของคุณ',
    noPortfolios: 'ยังไม่มีพอร์ตโฟลิโอ',
    createAndShare: 'สร้างพอร์ตโฟลิโอแรกและแชร์ให้โลกเห็น',
    newPortfolio: 'พอร์ตโฟลิโอใหม่',
    demoMode: 'โหมด Demo',
    demoBanner: 'คุณกำลังสำรวจ PortGen กับข้อมูลตัวอย่าง',
    signUpReal: 'สมัครบัญชีจริง →',
    exitDemo: 'ออกจาก Demo',
    signOut: 'ออกจากระบบ',
    edit: 'แก้ไข',
    viewOnly: 'ดูอย่างเดียว',
    live: 'เผยแพร่แล้ว',
    draft: 'ฉบับร่าง',
    views: 'views',
    upgradeToPro: 'อัพเกรดเป็น Pro',
  },

  // Builder
  builder: {
    new: 'พอร์ตโฟลิโอใหม่',
    editing: 'กำลังแก้ไข:',
    untitled: 'ไม่มีชื่อ',
    dashboard: 'แดชบอร์ด',
    jsxEditor: 'JSX Editor',
    preview: 'ตัวอย่าง',
    hidePreview: 'ซ่อนตัวอย่าง',
    save: 'บันทึก',
    publish: 'เผยแพร่',
    steps: {
      info: 'ข้อมูล',
      skills: 'ทักษะ',
      projects: 'โปรเจกต์',
      social: 'โซเชียล',
      theme: 'ธีม',
      preview: 'ตัวอย่าง',
    },
    info: {
      title: 'ข้อมูลพื้นฐาน',
      subtitle: 'เริ่มจากชื่อและแท็กไลน์ของคุณ',
      name: 'ชื่อ',
      namePlaceholder: 'สมชาย ดีเจริญ',
      tagline: 'แท็กไลน์',
      taglinePlaceholder: 'Full-Stack Developer • React & Node.js',
      about: 'เกี่ยวกับคุณ',
      aboutPlaceholder: 'บอกผู้เยี่ยมชมเกี่ยวกับตัวคุณ ประสบการณ์ และสิ่งที่คุณทำ...',
      avatarUrl: 'URL รูปโปรไฟล์',
      avatarUrlPlaceholder: 'https://example.com/your-photo.jpg',
    },
    skills: {
      title: 'ทักษะ',
      subtitle: 'เพิ่มทักษะด้านเทคโนโลยีของคุณ',
      placeholder: 'พิมพ์ทักษะ (เช่น React, Node.js) แล้วกด Enter',
      noSkills: 'ยังไม่มีทักษะ พิมพ์ด้านบนแล้วกด Enter',
    },
    projects: {
      title: 'โปรเจกต์',
      subtitle: 'โชว์ผลงานที่ดีที่สุดของคุณ',
      addProject: 'เพิ่มโปรเจกต์',
      noProjects: 'ยังไม่มีโปรเจกต์',
      addFirst: 'เพิ่มโปรเจกต์แรก',
      projectN: 'โปรเจกต์',
      remove: 'ลบ',
      titlePlaceholder: 'ชื่อโปรเจกต์',
      descPlaceholder: 'คำอธิบายโดยย่อ...',
      liveUrl: 'Live URL (ไม่บังคับ)',
      repoUrl: 'Repo URL (ไม่บังคับ)',
    },
    social: {
      title: 'ลิงก์โซเชียล',
      subtitle: 'เชื่อมต่อโปรไฟล์ออนไลน์ของคุณ',
      placeholder: 'https://...',
      add: 'เพิ่ม',
      platforms: {
        github: 'github',
        linkedin: 'linkedin',
        twitter: 'twitter',
        email: 'email',
        website: 'เว็บไซต์',
        facebook: 'facebook',
        instagram: 'instagram',
        youtube: 'youtube',
      },
    },
    theme: {
      title: 'เลือกธีม',
      subtitle: 'เลือกรูปแบบที่เป็นตัวคุณ',
    },
    previewStep: {
      title: 'ตัวอย่างและเผยแพร่',
      subtitle: 'ตรวจสอบพอร์ตโฟลิโอของคุณและเผยแพร่',
      willBeLive: 'พอร์ตโฟลิโอของคุณจะอยู่ที่:',
      whatsIncluded: 'สิ่งที่จะมี:',
      publishNow: 'เผยแพร่พอร์ตโฟลิโอ',
    },
    previous: 'ก่อนหน้า',
    stepOf: 'ขั้นตอน',
    of: 'จาก',
    pleaseAddName: 'กรุณาเพิ่มชื่อของคุณก่อน!',
    portfolioPublished: 'พอร์ตโฟลิโอเผยแพร่แล้ว! แชร์ลิงก์ได้เลย',
    jsxSaved: 'บันทึก JSX แล้ว! ตอนนี้คุณสามารถ export เป็น HTML ได้',
  },

  // Common
  common: {
    loading: 'กำลังโหลด...',
    error: 'เกิดข้อผิดพลาด',
    retry: 'ลองอีกครั้ง',
    close: 'ปิด',
    cancel: 'ยกเลิก',
    confirm: 'ยืนยัน',
    delete: 'ลบ',
    edit: 'แก้ไข',
    create: 'สร้าง',
    save: 'บันทึก',
    next: 'ถัดไป',
    back: 'กลับ',
    yes: 'ใช่',
    no: 'ไม่',
    free: 'ฟรี',
  },
}
